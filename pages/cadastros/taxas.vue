...
function save() {
  if (editingId.value) {
    ...
    if (existing) items.value[idx] = { ...existing, ...form.value, jurosMes: form.value.jurosMes ?? existing.jurosMes, multa: form.value.multa ?? existing.multa }
  } else {
    items.value.unshift({ id: String(Date.now()), ...form.value, jurosMes: form.value.jurosMes ?? 0, multa: form.value.multa ?? 0 })
  }
