...
function save() {
  if (editingId.value) {
    ...
    if (existing) items.value[idx] = { ...existing, ...form.value, taxa: form.value.taxa ?? existing.taxa }
  } else {
    items.value.unshift({ id: String(Date.now()), ...form.value, taxa: form.value.taxa ?? 0 })
  }
